  ...
  async buttonPressed() {
    ...
    else // User with given Id is valid(Present in the list)
    {
      ...
      if (obj === undefined && this.record.attendance === 'Present') { //If obj is undefined, we are pushing as it is. Because the obj for the given date is not present only.
        ...
        let response2 = await fetch(`https://6537e9dfa543859d1bb10641.mockapi.io/record`, {
          method: "POST",
          headers: {
      ...
      else if (obj !== undefined) { // If not undefined, which means date is already present we need to append new pid with existing pid array of the obj.
        ...
        if (this.record.pid.length === 0) {
          let response2 = await fetch(`https://6537e9dfa543859d1bb10641.mockapi.io/record/${obj.id}`, {
            method: "DELETE",
          });
        }
        else {
